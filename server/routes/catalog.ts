...
router.get('/catalog', async (req: Request, res: Response) => {
  try {
    const {
      ...
      sort = 'recent',
      search
    } = req.query as CatalogQueryParams;
 ...
 */
router.get('/catalog/stats/overview', async (req: Request, res: Response) => {
  try {
    ...
    const stats = {
      drops: data?.find((d: any) => d.item_type === 'drop')?.count || 0,
      products: data?.find((d: any) => d.item_type === 'product')?.count || 0,
      releases: data?.find((d: any) => d.item_type === 'release')?.count || 0
    };
 ...
 */
router.get('/catalog/:creator_id/creator', async (req: Request, res: Response) => {
  try {
    const { creator_id } = req.params;
    const { page = 1, limit = 50 } = req.query as CatalogQueryParams;
