...
const ArtistProfilePage = () => {
  ...
  if (isMobile) {
    return (
      <div className="min-h-[calc(100vh-88px)] bg-white pb-20">
        {/* Banner */}
        <div className="relative h-40 w-full overflow-hidden bg-[#dbeafe]">
          <img src={transformedArtist.banner} alt={transformedArtist.name} className="h-full w-full object-cover" />
          <button
            onClick={() => navigate(-1)}
            className="absolute top-4 left-4 z-10 rounded-full bg-white/90 p-2 text-foreground"
          >
            <ArrowLeft className="h-4 w-4" />
          </button>
          <button
            onClick={handleShare}
            className="absolute top-4 right-4 z-10 rounded-full bg-white/90 p-2 text-foreground"
          >
        ...
        <div className="px-4 pt-4">
          <div className="flex items-end gap-3 mb-4">
            ...
            />
            <div className="mb-1">
              <h1 className="text-2xl font-black text-foreground">{transformedArtist.name}</h1>
              <p className="text-sm text-muted-foreground">{transformedArtist.tag}</p>
            </div>
          </div>

          <p className="text-sm text-muted-foreground leading-6 mb-4">{transformedArtist.bio}</p>

          {/* Stats */}
          <div className="grid grid-cols-2 gap-3 mb-4">
            <div className="rounded-xl bg-[#f3f8ff] p-3">
              <p className="text-lg font-black text-[#1d4ed8]">{isSubscribersLoading ? "..." : onchainSubscribers}</p>
              <p className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Subscribers</p>
            </div>
            <div className="rounded-xl bg-[#f3f8ff] p-3">
              <p className="text-lg font-black text-[#1d4ed8]">{drops.length}</p>
              <p className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Drops</p>
            </div>
          </div>
          ...
          <div className="flex gap-2 mb-4 flex-wrap">
            {publicLinks.map((link) => (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex items-center gap-1 rounded-full bg-[#f3f8ff] px-3 py-1.5 text-xs text-[#1d4ed8]"
              >
            ...
            >
              {isConnected
                ? isSubscribed
                  ...
                  : isSubscribedLoading
                    ? "Checking..."
                    : isSubscribing || isSubscribePending || isSubscribeConfirming
                      ? "Processing..."
        ...
        <div className="px-4">
          <Tabs defaultValue="portfolio" className="w-full">
            <TabsList className="grid w-full grid-cols-2 bg-[#f3f8ff] rounded-full">
              <TabsTrigger value="portfolio" className="rounded-full">
                Portfolio
              </TabsTrigger>
              <TabsTrigger value="drops" className="rounded-full">
                Drops
              </TabsTrigger>
            </TabsList>

            <TabsContent value="portfolio" className="mt-4 mb-8">
            ...
            <TabsContent value="drops" className="mt-4 mb-8">
              {dropsLoading ? (
                ...
                <div className="grid grid-cols-1 gap-3">
                  {drops.map((drop) => (
                    <Link key={drop.id} to={`/drops/${drop.id}`} className="overflow-hidden rounded-2xl bg-white border border-[#dbeafe]">
                      ...
                      <div className="p-3">
                        ...
                        <div className="mt-2 flex items-center gap-3 text-xs text-muted-foreground">
                          <span className="inline-flex items-center gap-1"><Flame className="h-3 w-3 text-[#1d4ed8]" /> {drop.priceEth} ETH</span>
                          <span className="inline-flex items-center gap-1"><Users className="h-3 w-3 text-[#1d4ed8]" /> {drop.bought}/{drop.maxBuy}</span>
                        </div>
                      </div>
                    </Link>
                  ))}
  ...
  const handleDesktopSubscribe = async () => {
    if (!isConnected) {
      await connectWallet();
      return;
    }
        ...
        <div className="grid gap-5 lg:grid-cols-[320px_minmax(0,1fr)] xl:grid-cols-[340px_minmax(0,1fr)]">
          <aside className="rounded-[1.8rem] bg-[linear-gradient(180deg,#60a5fa_0%,#1d4ed8_100%)] p-5 text-white shadow-[0_24px_60px_rgba(37,99,235,0.28)]">
            ...
            <div className="mt-6 space-y-3">
              <Button
                onClick={handleDesktopSubscribe}
                disabled={isSubscribing || isSubscribePending || isSubscribeConfirming || isSubscribed || isSubscribedLoading}
